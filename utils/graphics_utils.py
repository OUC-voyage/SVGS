...
import cv2 # 新增
import os # 新增
from scaffold_gs_pro import propagate # 新增
...
def write_cam_txt(cam_path, K, w2c, depth_range): # 新增
...
def init_image_coord(height, width): # 新增
...
def depth_to_xyz(depth, intrinsic): # 新增
    b, c, h, w = depth.shape
...
def get_surface_normalv2(xyz, patch_size=5): # 新增
    """
    xyz: xyz coordinates
    patch: [p1, p2, p3,
            p4, p5, p6,
...
def surface_normal_from_depth(depth, intrinsic, valid_mask=None): # 新增
...
def img_warping(ref_pose, src_pose, virtual_pose_ref_depth, virtual_intrinsic, src_img): # 新增
...
def get_proj_matrix(K, image_size, znear=.01, zfar=1000.): # 新增
    fx = K[0,0]
    fy = K[1,1]
    cx = K[0,2]
    cy = K[1,2]
...
def bilinear_sampler(img, coords, mask=False): # 新增
...
def reproject_with_depth(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src): # 新增
