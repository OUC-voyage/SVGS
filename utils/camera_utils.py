...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    if args.resolution in [1, 2, 4, 8]:
        ...
        K = cam_info.K / scale # 新增

    resized_image_rgb = PILtoTorch(cam_info.image, resolution)
    gt_image = resized_image_rgb[:3, ...]
    resized_depth_anything = PILtoTorch(cam_info.depth_anything, resolution)
    gt_depth_anything = resized_depth_anything[:3, ...]

    if cam_info.sky_mask is not None: # 新增
    ...
    if cam_info.normal is not None: # 新增
    ...
    if cam_info.depth is not None: # 新增
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  ...
                  image=gt_image, gt_depth=gt_depth_anything, gt_alpha_mask=loaded_mask,
