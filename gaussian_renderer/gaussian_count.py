...
"""
def generate_neural_gaussians1(viewpoint_camera, pc : GaussianModel, visible_mask=None, is_training=False):
    ## view frustum filtering for acceleration    
    if visible_mask is None:
        visible_mask = torch.ones(pc.get_anchor.shape[0], dtype=torch.bool, device = pc.get_anchor.device)
    ...
    feat = pc._anchor_feat[visible_mask]
    anchor = pc.get_anchor[visible_mask]
    grid_offsets = pc._offset[visible_mask] #(N, 10, 3)
    ...
    if pc.add_cov_dist:
        ...
        scale_rot = pc.get_cov_mlp(cat_local_view_wodist) #(N, 70)
    ...
    scaling_repeat, repeat_anchor, scale_rot, offsets = concatenated_all.split([6, 3, 7, 3], dim=-1)
    
    # post-process cov
    scaling = scaling_repeat[:,3:] * torch.sigmoid(scale_rot[:,:3]) # * (1+torch.sigmoid(repeat_dist))
    scaling = scaling.reshape([anchor.shape[0], pc.n_offsets, 3])
    rot = pc.rotation_activation(scale_rot[:,3:7])
    rot = rot.reshape([anchor.shape[0], pc.n_offsets, 4])
    
    # post-process offsets to get centers for gaussians
    offsets = offsets * scaling_repeat[:,:3]
    xyz = repeat_anchor + offsets

    offsets = offsets.reshape([anchor.shape[0], pc.n_offsets, 3])
    xyz = xyz.reshape([anchor.shape[0], pc.n_offsets, 3])

    return xyz, opacity, scaling, rot, neural_opacity
"""
