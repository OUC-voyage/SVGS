...
class CameraInfo(NamedTuple):
    ...
    image_path: str
    image_name: str
    depth_anything: np.array
    depth_path_anything: str
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, depths_folder, sky_seg=False, load_normal=False, load_depth=False):
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):
        ...
        depth_path_anything = os.path.join(depths_folder, os.path.basename(extr.name))
        depth_name_anything = os.path.basename(depth_path_anything).split(".")[0]
        #depth_path = depths_mapping[idx]
        #depth_anything = Image.open(depth_path)
        depth_anything = Image.open(depth_path_anything)
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              image_path=image_path, image_name=image_name, depth_anything=depth_anything, depth_path_anything=depth_path_anything, depth_name_anything=depth_name_anything, width=width, height=height,
...
def readColmapSceneInfo(path, images, depths, eval, lod, llffhold=8, sky_seg=False, load_normal=False, load_depth=False):
    try:
    ...
    #depths_mapping = [f for f in sorted(glob.glob(os.path.join(path, depths_dir, '*')),
    #                             key=lambda x: int(re.findall(r'\d+', os.path.basename(x))[0])) 
    #           if f.endswith(('JPG', 'jpg', 'png'))]
    #cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir), rgb_mapping=rgb_mapping, depths_folder=os.path.join(path, depths_dir), depths_mapping = depths_mapping,
    #                                       sky_seg=sky_seg, load_normal=load_normal, load_depth=load_depth)
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir), depths_folder=os.path.join(path, depths_dir),
                                           sky_seg=sky_seg, load_normal=load_normal, load_depth=load_depth)
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", is_train=True, is_debug=False, undistorted=False):
    cam_infos = []
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            if undistorted:
                ...
                image = Image.fromarray(np.array(arr*255.0, dtype=np.byte), "RGB")
                sky_mask = np.ones_like(image)[:, :, 0].astype(np.uint8) #新增
